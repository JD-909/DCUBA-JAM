...
const character_frames : Array[int] = [64, 86, 53, 75, 20, 64, 64]
...
@onready var helm_sprite = $HelmSprite
...
func load_character() -> void:
	...
	if Global.current_character == 5:
		$HelmSprite.visible = true
	else:
		$HelmSprite.visible = false
	
	if Global.current_character == 6:
		$NegomaSprite.visible = true
	else:
		$NegomaSprite.visible = false
