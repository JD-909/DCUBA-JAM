...
func _ready() -> void:
	...
	$VBoxContainer/HBoxContainer2/DrawLimitStar/Sprite2D/Label.text += str(Global.drawings) +"/"+ str(Global.draw_limits[checking_level[0]][checking_level[1]])
	$VBoxContainer/HBoxContainer2/EraseLimitStar/Sprite2D/Label.text += str(Global.erasures) +"/"+ str(Global.erase_limits[checking_level[0]][checking_level[1]])
	$VBoxContainer/HBoxContainer2/EraseAllStar/Sprite2D/Label.text += str(Global.current_permanents_amount -1) + " sin borrar)"
	
	# Star color handling
	if not Global.level_done_draw_limit[checking_level[0]][checking_level[1]]:
		$VBoxContainer/HBoxContainer2/DrawLimitStar/Sprite2D.modulate = Color("555555ff")
	if not Global.level_done_erase_limit[checking_level[0]][checking_level[1]]:
		$VBoxContainer/HBoxContainer2/EraseLimitStar/Sprite2D.modulate = Color("555555ff")
	if not Global.level_done_erase_all[checking_level[0]][checking_level[1]]:
		$VBoxContainer/HBoxContainer2/EraseAllStar/Sprite2D.modulate = Color("555555ff")
	if not Global.level_done_secret[checking_level[0]][checking_level[1]]:
		$VBoxContainer/HBoxContainer2/SecretStar/Sprite2D.modulate = Color("555555ff")

func _on_redo_pressed() -> void:
	get_tree().change_scene_to_file(Global.levels_by_ID[checking_level[0]][checking_level[1]])
